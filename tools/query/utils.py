...
def add_time_range_filter(base_query: str, context: AnalysisContext) -> str:
    ...
    if context.time_range:
        ...
        if start_date:
            base_query += f" AND DATE(o.created_at) >= '{start_date}'"
        if end_date:
            base_query += f" AND DATE(o.created_at) <= '{end_date}'"
    return base_query
...
def add_default_time_filter(base_query: str, default_days: int = 730) -> str:
    ...
    return base_query + f" AND DATE(o.created_at) >= DATE_SUB(CURRENT_DATE(), INTERVAL {default_days} DAY)"